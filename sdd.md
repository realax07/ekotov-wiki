...
Индексы: `tasks(status, is_fast)`, `tasks(archived_at)`, `tasks(priority)`, `task_tags(tag_id)`, `comments(task_id)`, `tasks(done_at)` (фильтр ленивой автоархивации и столбца done, §3.3).
