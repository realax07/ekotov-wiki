...
Версия sdd: r4 (2026-09-17) — r3 (37a7a21) + /api/health в §3 и exempt-список авторизации (эскалация из ревью задачи 1.1, `code-reviews/add-kanban-core/review-1.1-001.md`, замечания 1–2).
...
`GET /login`, `GET /board`, `GET /search`, `GET /wiki` (заглушка с todo). Все, кроме `/login` и раздаваемой nginx'ом статики, — редирект на `/login` без сессии (страница входа и статика должны быть доступны без сессии, иначе вход невозможен; exemption страниц — не часть exempt-списка API-авторизации выше).
