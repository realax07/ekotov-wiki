...
лексикографическое по ISO-строкам: ОБЕ строки — одного формата
«YYYY-MM-DDTHH:MM:SS+03:00» (done_at и граница — из одного источника
app.board.msk_now_iso / _msk_day_start_iso; blocker review-6.1-001:
формат «+00:00» у done_at ломал лексикографический порядок в
00:00–03:59 МСК). Строки одного формата сортируются хронологически.
При любом серверном TZ: datetime.now(tz) дает один и тот же момент, а
граница всегда 00:00 UTC+3.
...
def msk_now_iso() -> str:
    """Текущий момент в фиксированном МСК (UTC+3), ISO-строка «...+03:00».

    Единый источник МСК-времени для всех календарно-сравниваемых полей
    tasks: done_at (move, app/tasks.py) и границы дня (autoarchive,
    здесь). Сравнение с done_at в autoarchive — лексикографическое по
    ISO-строкам; оно хронологически корректно ТОЛЬКО при одном формате
    смещения у обеих строк (blocker review-6.1-001: done_at в «+00:00»
    при границе «+03:00» ломает порядок в 00:00–03:59 МСК). Поэтому
    done_at обязан писаться этой же функцией.
    """
    return datetime.now(timezone.utc).astimezone(MSK).isoformat()
...
def autoarchive_done_tasks(conn: sqlite3.Connection) -> None:
    ...
    граница — ISO-строки одного формата (UTC+03:00, обе через
    msk_now_iso/_msk_day_start_iso), сравнение лексикографическое =
    хронологическое.
    """
    conn.execute(
